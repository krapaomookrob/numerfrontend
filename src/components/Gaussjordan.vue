        ...
        <v-slide-group multiple show-arrows>
          <v-slide-item v-for="m in mongo" :key="m._id">
            <v-card @click="settoans(m)">
              <v-card-title>Matrix A{{m.matrixgordan}}</v-card-title>
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col>
  ...
  methods: {
    ...
    async initmatrix() {
      ...
      for (let l = 0; l < size; l++) {
        arrayx[l].push(parseInt(document.getElementById(`b${l + 1}`).value));
      }
      ...
      await axios
        ...
        .then(res => {
          ...
          for (let i = 0; i < res.data.result.length; i++) {
            console.log(res.data.result[i][size]);
            temp.push(res.data.result[i][size]);
          }
          console.log(temp);
          this.answers = temp;
        })
